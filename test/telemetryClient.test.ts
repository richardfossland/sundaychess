...
function seedSession(p: {
  ...
  store.set(
    `sjakk:player:${p.tournamentId}`,
    JSON.stringify({ resumeCode: "KOLE-7F", displayName: "Ada", ...p }),
  );
  store.set("sjakk:player:last", p.tournamentId);
  const index = new Set<string>(JSON.parse(store.get("sjakk:player:index") ?? "[]"));
  index.add(p.tournamentId);
  store.set("sjakk:player:index", JSON.stringify([...index]));
}
...
describe("attribution (R6)", () => {
  ...
  it("stamps nothing from /solo, /versus or /host, whatever the kind", () => {
    for (const path of ["/solo", "/versus", "/host/abc", "/"]) {
      sendBeacon.mockClear();
      __resetTelemetry();
      installBrowser({ path });
      report("channel_error", { status: "TIMED_OUT" });
