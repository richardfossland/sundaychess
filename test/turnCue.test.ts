...
describe("nextFlashTitle — alternation sequence", () => {
  const original = "SundayChess — sjakkturnering for hele gruppa";
  const flash = "▶ Din tur! – SundayChess";
...
describe("fireDeviceCues — notification, opt-in only", () => {
  ...
  it("shows a tagged, non-silent notification once permission is granted", () => {
    ...
    expect(instances[0].options).toMatchObject({ tag: "sjakk-turn", silent: false });
...
describe("notify opt-in persistence — sjakk:notify", () => {
  class MemStorage {
  ...
  it("uses the documented key", () => {
    expect(NOTIFY_STORAGE_KEY).toBe("sjakk:notify");
  });
