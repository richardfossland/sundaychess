...
// L5: these three sit in front of a setState that used to fire unconditionally
// on a timer. A false positive (saying "same" when something changed) would
// DROP a real update, so every field the UI reads has to be covered.
...
function detail(over: Partial<GameDetail> = {}): GameDetail {
  return {
    ...
    lastMove: null,
    drawOfferedBy: null,
    clock: { whiteMs: 300000, blackMs: 300000, turn: "w", running: true },
    ...over,
  };
}
...
describe("sameDetail", () => {
  ...
  it.each([
    ["id", { id: "g2" }],
    ["fen", { fen: "8/8/8/8/8/8/8/K6k w - - 0 1" }],
    ["pgn", { pgn: "1. e4" }],
    ...
    ["lastMove", { lastMove: { from: "e2", to: "e4", san: "e4" } }],
    ["clock -> null", { clock: null }],
  ])("is false when %s changes", (_label, over) => {
    expect(sameDetail(detail(), detail(over as Partial<GameDetail>))).toBe(false);
  });

  it("compares every field of lastMove, not just the SAN", () => {
    const base = detail({ lastMove: { from: "e2", to: "e4", san: "e4" } });
    expect(sameDetail(base, detail({ lastMove: { from: "e2", to: "e4", san: "e4" } }))).toBe(true);
    expect(sameDetail(base, detail({ lastMove: { from: "d2", to: "e4", san: "e4" } }))).toBe(false);
    expect(sameDetail(base, detail({ lastMove: { from: "e2", to: "e3", san: "e4" } }))).toBe(false);
    expect(sameDetail(base, detail({ lastMove: { from: "e2", to: "e4", san: "e4+" } }))).toBe(false);
  });

  it("compares every field of the clock", () => {
    const base = detail();
    expect(
      sameDetail(base, detail({ clock: { whiteMs: 299000, blackMs: 300000, turn: "w", running: true } })),
    ).toBe(false);
    expect(
      sameDetail(base, detail({ clock: { whiteMs: 300000, blackMs: 299000, turn: "w", running: true } })),
    ).toBe(false);
    expect(
      sameDetail(base, detail({ clock: { whiteMs: 300000, blackMs: 300000, turn: "b", running: true } })),
    ).toBe(false);
    expect(
      sameDetail(base, detail({ clock: { whiteMs: 300000, blackMs: 300000, turn: "w", running: false } })),
    ).toBe(false);
  });

  it("treats an ABSENT optional field the same as an explicit null", () => {
    // The API omits drawOfferedBy/clock on some paths and sends null on
    // others; a shape difference alone must not read as a change.
    const withNulls = detail({ drawOfferedBy: null, clock: null });
    const absent = detail();
    delete (absent as Partial<GameDetail>).drawOfferedBy;
    delete (absent as Partial<GameDetail>).clock;
    expect(sameDetail(withNulls, absent)).toBe(true);
    expect(sameDetail(absent, withNulls)).toBe(true);
  });

  it("still sees a real change when both sides omit the optional fields", () => {
    const a = detail();
    const b = detail({ status: "white_win" });
    for (const d of [a, b]) {
      delete (d as Partial<GameDetail>).drawOfferedBy;
      delete (d as Partial<GameDetail>).clock;
    }
