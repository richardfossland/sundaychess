...
import { readSolved } from "@/lib/client/PuzzleCard";
...
describe("storage guards — unavailable localStorage (SecurityError)", () => {
  ...
  it("PuzzleCard's readSolved() falls back to the 0 default instead of throwing", () => {
    expect(() => readSolved()).not.toThrow();
    expect(readSolved()).toBe(0);
  });
});
...
describe("storage guards — working localStorage", () => {
  ...
  it("readSolved() reflects what was persisted under its key", () => {
    expect(readSolved()).toBe(0);
    safeSet("sundaychess:puzzles-solved", "3");
    expect(readSolved()).toBe(3);
  });
});
