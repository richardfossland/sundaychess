...
/**
 * L5: equality helpers whose ONLY job is to keep a reference stable when the
 * value behind it did not change.
 *
 ...
 * the wire — stringifying it is far cheaper than the React re-render (plus 64
 * squares and pieces) it prevents every 5 s, and both sides come from
 ...
 * (explicitly none) compare equal — the API omits `drawOfferedBy`/`clock`
 * entirely on some routes and sends `null` on others. */
export function sameDetail(a: GameDetail | null, b: GameDetail | null): boolean {
  ...
  return (
    ...
    (a.lastMove?.from ?? null) === (b.lastMove?.from ?? null) &&
    (a.lastMove?.to ?? null) === (b.lastMove?.to ?? null) &&
    (a.lastMove?.san ?? null) === (b.lastMove?.san ?? null) &&
    (a.clock?.whiteMs ?? null) === (b.clock?.whiteMs ?? null) &&
    (a.clock?.blackMs ?? null) === (b.clock?.blackMs ?? null) &&
    (a.clock?.turn ?? null) === (b.clock?.turn ?? null) &&
    (a.clock?.running ?? null) === (b.clock?.running ?? null)
  );
}
