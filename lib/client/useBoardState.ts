...
export function useBoardState(tournamentId: string | null, withClocks = false) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      // L5: keep the PREVIOUS object when the board is byte-identical. This
      // poll fires every 5 s and used to hand every consumer a new `state`
      // reference each time — in the player app that re-rendered WaitingRoom
      // and, with it, the whole live board. BoardState is a few KB of plain
      // JSON straight off the wire, so `JSON.stringify` equality is far
      // cheaper than the render it prevents (see lib/client/equal.ts).
      // `error`/`errorStatus`/`errorCode`/`failures` are untouched below: they
      // already carry primitive values, so React bails out on its own when
      // they are reset to the same value.
