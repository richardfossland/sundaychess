 ...
 * `Set` each time; L5 keeps the previous reference when MEMBERSHIP is
 * unchanged, so one student's phone waking up no longer re-renders every other
 * client's tree (in the player app that parent is WaitingRoom, whose child is
 * the live board). Shares the one memoised browser client / socket. */
