...
//
// ## Why student sessions are keyed PER TOURNAMENT (R6)
//
// The old layout was a single `sjakk:player` slot: one student session per
...
//
// So: one record per tournament under `sjakk:player:<tournamentId>`, plus a
// `sjakk:player:last` pointer naming the tournament the bare `/play` entry
// should resume, plus a `sjakk:player:index` list so `allPlayers()` can
...
/**
 * Pre-R6 devices carry one `sjakk:player` blob. Fold it into the new layout the
