...
alter table public.games  add column if not exists slot int not null default 0;
alter table public.rounds add column if not exists extended_ms int not null default 0;

create or replace function public.extend_round(p_round_id uuid)
returns int
language sql
as $$
