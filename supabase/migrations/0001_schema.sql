...
create index tournaments_join_pin_idx on public.tournaments (join_pin);
...
create index rounds_tournament_idx on public.rounds (tournament_id);
...
create table public.games (
  ...
  black_player_id uuid references public.players (id),   -- null = bye
  fen             text not null,
...
create index games_round_idx on public.games (round_id);
...
create table public.moves (
  ...
  by_player_id  uuid not null references public.players (id),
  created_at    timestamptz not null default now(),
  unique (game_id, ply)
);
