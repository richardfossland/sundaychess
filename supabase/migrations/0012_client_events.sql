...
create index if not exists client_events_tournament_idx
  on public.client_events (tournament_id, at desc);

-- ---------- RLS: lock to the service role, like every other table (0001) ----------
...
-- to read this is through the host-code-authenticated diagnostics route.

-- ---------- retention: 14 days, no exceptions ----------
-- Mirrors 0004's style: a security-definer function plus an idempotent
-- (unschedule-then-schedule) pg_cron job, so this migration is safe to re-run.

create extension if not exists pg_cron;

create or replace function public.cleanup_client_events()
returns integer
language plpgsql
security definer
...
do $$
begin
  perform cron.unschedule('cleanup-client-events');
exception when others then
  null; -- wasn't scheduled yet
end $$;

-- 04:20 UTC — a few minutes after the tournament sweep (0004/0010) so the two
-- nightly jobs don't start in the same second.
select cron.schedule(
  'cleanup-client-events',
  '20 4 * * *',
  $$select public.cleanup_client_events();$$
);
