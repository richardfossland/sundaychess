...
begin
  ...
  select coalesce(max(ply), 0) + 1 into v_ply from public.moves where game_id = p_game_id;

  insert into public.moves (game_id, ply, san, fen_after, by_player_id)
...
create or replace function public.recompute_scores(p_tournament_id uuid)
returns void
language plpgsql
as $$
begin
  update public.players p set score = coalesce(s.total, 0)
  from (
    select pid, sum(pts) as total from (
      select white_player_id as pid,
        ...
        from public.games where tournament_id = p_tournament_id
      union all
