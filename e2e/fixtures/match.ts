...
//     seam has not drifted away from the real join flow. At least one spec must
//     take it, or the whole suite is testing a shortcut nobody ships.

export interface Match {
  tournamentId: string;
  gameId: string;
  /** Host bearer code (quickmatch only; `null` from the public flow helper's
   *  perspective it is available, so both carry it for symmetry). */
  hostCode: string;
  white: StoredPlayer;
  black: StoredPlayer;
}
 ...
 * the pairing made white. Deliberately mirrors scripts/smoke-features.mjs, so
 * one spec can prove the seam's shortcut lands a player in the same place the
 * real flow does.
 */
export async function publicFlowMatch(
  ...
  // The pairing decides the colours, not the join order.
 ...
 *
 * Resolves once `board-shell` is on screen — the board is the only proof that
 * all three steps completed.
 *
 * `legacy: true` seeds the PRE-R6 single-slot `sjakk:player` key instead of the
