...
test("a 503 edge page on every game poll: the board stays, and it heals", async ({
  ...
  try {
    ...
    while (Date.now() - start < 15_000) {
      ...
      expect(await board.pieceAt("e2")).toBe("wP");
...
test("a move POST that never answers rolls back inside the fetch deadline", async ({
  ...
  try {
    ...
    try {
      await board.clickMove("e2", "e4");
      await expect.poll(() => board.pieceAt("e4"), { timeout: 3_000 }).toBe("wP");
      ...
      await expect
        .poll(() => board.pieceAt("e2"), { timeout: 11_000 })
        .toBe("wP");
      expect(await board.pieceAt("e4")).toBeNull();
      await expect(board.shell()).toBeVisible();
...
test("the board still opens once the edge stops answering for it", async ({
  ...
  try {
    ...
    expect(await new BoardPage(page).pieceAt("e2")).toBe("wP");
  } finally {
    await Promise.all(contexts.map((c) => c.close()));
  }
});
