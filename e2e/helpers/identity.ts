...
// before ANY app code runs and `/play` (or `/host/<id>`) walks its real path.
// Reading is how a spec proves the opposite of a bug: after a 503, a WAF 403 or
// a dropped network, the student's resume code must still be on the device.
//
// R6: student sessions are keyed PER TOURNAMENT — `sjakk:player:<tournamentId>`
// plus a `sjakk:player:last` pointer and a `sjakk:player:index` list. The old
// single-slot `sjakk:player` is migration input only; `seedLegacyPlayer` writes
...
/** The same identity in the PRE-R6 layout: one `sjakk:player` blob and nothing
 ...
 * writes when the browser created the tournament. `/host/<id>` reads it on mount
 * and every host action (start round, kick) is refused without it, so a spec that
 * asserts the host did NOT kick anyone is vacuous unless this ran first.
 */
