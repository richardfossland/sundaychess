...
// moves with the same bearer pair, the server's optimistic-FEN check rejects the
// loser, and the student experiences it as "I can't move my pieces". So exactly
// one tab is the board; the rest show "Spill her".
//
...
test("only the newest tab is the board, and a released one hands it back", async ({
  ...
  try {
    ...
    await firstBoard.clickMove("e2", "e4");
    await expect.poll(() => firstBoard.pieceAt("e4"), { timeout: 10_000 }).toBe("wP");
  } finally {
