...
// R6 made student sessions per-tournament (`sjakk:player:<tournamentId>` + a
// `sjakk:player:last` pointer). Every device in a classroom that is mid-
// tournament when that deploy lands is carrying the OLD single-slot
// `sjakk:player` blob instead.
//
...
test("a pre-R6 device keeps its seat: legacy key resumes, then is gone", async ({
  ...
  try {
    ...
    // `legacy: true` seeds ONLY `sjakk:player`, exactly as pre-R6 code wrote it.
    // openAs then walks the real path — /play → attemptResume → WaitingRoom
    // latches the live game → GameView — and returns only once 64 squares are on
    // screen. Reaching the board at all is the migration working.
    ...
    expect(
      ...
      "the legacy sjakk:player key survived the migration",
    ).toBeNull();
